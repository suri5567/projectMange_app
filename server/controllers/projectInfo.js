...
//    try{
// 	 const user = await ProjectDetailschema.findByIdAndUpdate(id, {data}); 
// 	 if(!user){
// 		 res.status(404).json({msg:"Project not found!!!"})
// 	 }else{
//         res.status(201).json({msg:"Project successfully Updated !!!"})
// 	 }
//    }catch(error){
// 	 res.status(500).json({msg:"Internal server error"})
//    } 
...
export const handleEditExitingProject = async (req, res) => {
	...
	try {
	  const updatedProject = await ProjectDetailschema.findByIdAndUpdate(
		id,
		{ status },
		{ new: true }
	  );
